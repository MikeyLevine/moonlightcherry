...
**Bug fix (Phase 20), found by the user testing production for real:** `sendMessage` never fired a notification at all — a recipient had zero site-wide indication of a new message unless they happened to already be sitting on the `/messages/[id]` polling page, since the conversation-list "unread indicator" only shows up if you navigate there yourself. Added a `MESSAGE` notification type, fired to the other participant on every send (`src/lib/messaging/actions.ts`), with its own `conversationId`-based `href` resolution in `src/lib/notifications/queries.ts` so it links straight to the conversation rather than falling through to a generic `/notifications` link. Verified end-to-end with two real accounts, not assumed: sent a real message via the actual Server Action RPC, confirmed the `Notification` row, the header bell's unread count incrementing, and the rendered notification text/link — then cleaned up all test data.
...
**Implemented (Phase 22): Settings.** `/admin/settings` (`src/lib/site-settings.ts`, `src/lib/admin/settings-actions.ts`) — upload size/dimension limits and storage warn/block thresholds (already `SiteSetting`-backed since Phase 5/6 with no UI to actually set them until now), plus every rate-limit *count* added in Phase 17 (window durations stay fixed in code — the count is the dial an admin would actually want to turn). One registry (`SETTINGS_REGISTRY`) is the single source of truth for every setting's key, label, default, and validation bounds, read by both the admin form and the code paths that enforce it — no second set of magic numbers to drift out of sync. Reads are cached for 30s (`unstable_cache`, tagged) since these are read on every rate-limited request and a live DB round-trip per comment/message/upload would be a real cost for something that changes rarely; saving calls Next 16's `updateTag` (not `revalidateTag`, which defaults to stale-while-revalidate as of this Next version — confirmed against the actual docs, not assumed, after `revalidateTag`'s signature changed under us) so a saved change is guaranteed live on the very next request, not eventually. Verified directly, not assumed: dropped the comment rate limit to 2 through the real admin UI, then posted 3 real comments and confirmed exactly 2 succeeded and the 3rd was rejected — proving the override actually reaches the request path, immediately, not just that the form saves.

**Implemented (Phase 21): Tags/Characters/Series.** `/admin/tags`, `/admin/characters`, `/admin/series` (`src/lib/admin/taxonomy-actions.ts`). Rename/merge/delete for all three; create + edit (series link, avatar, description) for characters; create + edit (type, cover, description) for series. Approve/reject suggestions is still not applicable — the suggestion-queue tables remain unused by design (§12's still-open item). Merge is the part worth being precise about: reassigns every join-table row (`MediaTag`/`MediaCharacter`/`MediaSeries`) from the merged-away record to the surviving one, **including de-duplicating first** when a media item already has both (verified directly, not assumed — set up a real upload with both a source and target tag already attached, merged them, and confirmed exactly one row survived rather than a constraint error or a silent duplicate); series merge additionally reassigns every character pointing at the merged-away series, not just media, which is easy to miss since a series' children aren't only "series → media" but also "series → character → media." Rename/merge/delete all write an `AuditLog` row. Found and fixed a real pre-existing data gap while verifying this, not left for later: `syncMediaAssociations` (the upload-time find-or-create path) had never set a character's `seriesId` even when a series was entered in the same upload — the Rukia Kuchiki test character had been sitting with no series link since Phase 9 despite "Bleach" existing as its series. Fixed both the one existing record (via the new edit UI, confirmed live on its public `/characters/[slug]` page) *and* the root cause in `syncMediaAssociations` itself, so it doesn't keep happening — series is now resolved before the character upsert so a brand-new character picks it up at creation, while an *existing* character's series is still never touched by a later upload (verified with two real uploads: one creating the link, one confirming it isn't clobbered).
...
**Implemented (Phase 19): production config artifacts, then actually deployed.** Everything under `deploy/` started as a checked-in config file the user would run manually (`deploy/README.md`) — installing Caddy, opening/restricting firewall ports, and enabling systemd services all require root and touch external-facing state, the kind of hard-to-reverse, shared-system change that gets a human's go-ahead rather than being done silently. With that go-ahead given, the checklist was actually executed end-to-end and **`https://moonlightcherry.xyz` is live**: separate production checkout (`/srv/moonlightcherry/app`, never the dev directory), a genuinely separate Postgres database *and role* (`moonlightcherry_prod` — real credential isolation from dev, not just a different database under the same login, since a docker-compose password override can't separate two databases sharing one already-initialized Postgres role), the app on port 3001 (avoiding the conflict a naive same-port deploy would have hit against dev's 3000), Caddy terminating TLS with a real Cloudflare Origin CA certificate (installed directly on the server by the user via `sudo vi`, not pasted through chat — the private key was still handled carefully: written once, moved into place with correct root/caddy ownership, and shredded from scratch storage immediately after), and the firewall restricted to Cloudflare's IP ranges only, verified live (not assumed) by checking `sudo ufw status` before and after. **A real bug was caught during launch verification, not left for a user to discover**: `/auth/providers` on production returned OAuth callback URLs pointing at the internal `localhost:3001` bind address instead of the public domain — `trustHost: true` alone doesn't reliably survive header-based inference behind a reverse proxy, confirmed against Auth.js's own deployment docs rather than guessed at. Fixed with an explicit `AUTH_URL` env var, verified by re-checking `/auth/providers` before and after the fix. Dev never surfaced this because `next dev -H dev.moonlightcherry.xyz` binds directly to the real hostname, sidestepping the whole header-inference path that only exists behind a proxy. Also caught and fixed two of this phase's own planning mistakes before they caused damage: the original systemd unit had no port override (would have conflicted with dev's 3000) and originally pointed at the dev working directory — both fixed before the first deploy attempt, the port conflict was actually hit once (`EADDRINUSE`) and corrected. What's in `deploy/`: a production `Caddyfile` (Cloudflare-proxied, `moonlightcherry.xyz` canonical + `www` → bare-domain redirect, per the user's explicit decision — using a **Cloudflare Origin CA certificate**, not Caddy's automatic Let's Encrypt, since automatic ACME's HTTP-01 challenge doesn't play well with a Cloudflare-proxied record; the Origin CA cert is the standard fix and needs no code, just a dashboard step); systemd units for the app and worker (`worker` runs plain `tsx`, not `tsx watch` — the file-watching is a dev convenience with a real cost in production); `deploy/deploy.sh` (pull → install → `prisma db push` → build → restart, matching the `db push`-not-migrations workflow this whole project has used); `deploy/backup.sh` + a daily systemd timer (Postgres dump + media directory tarball, with an optional off-site rclone destination — **local-only by default**, which the script says explicitly rather than implying real durability it doesn't have); and `deploy/allow-cloudflare-only.sh`, which restricts ports 80/443 to Cloudflare's published IP ranges — not just hardening, but a real requirement here specifically, since the CSAM Scanning Tool (§4/§14) only sees traffic that actually passes through Cloudflare's cache, and traffic that reaches the origin IP directly bypasses it entirely. Also moved local media storage to a path outside the app directory (`/srv/moonlightcherry/storage/media`) in the checklist, closing the loop on the storage-durability gap flagged in Phase 18 — the app directory itself is not a safe place for anything that needs to survive a redeploy.
